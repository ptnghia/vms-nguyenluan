 ...
 */
router.get('/status', requireOperator, async (req: Request, res: Response) => {
  try {
    // Get PM2 processes
    ...
    const camerasResult = await pool.query(
      "SELECT COUNT(*) as total, SUM(CASE WHEN status = 'active' THEN 1 ELSE 0 END) as active FROM cameras"
    );
    const cameras = {
 ...
 */
router.get('/stats', requireOperator, async (req: Request, res: Response) => {
  try {
    const stats = await systemService.getSystemStats();
    const gpu = await systemService.getGPUStats();
    ...
    const formatted = {
      ...
      memory: {
        ...stats.memory,
        totalFormatted: systemService.formatBytes(stats.memory.total),
        usedFormatted: systemService.formatBytes(stats.memory.used),
        freeFormatted: systemService.formatBytes(stats.memory.free)
      },
      disk: {
        ...stats.disk,
        totalFormatted: systemService.formatBytes(stats.disk.total),
        usedFormatted: systemService.formatBytes(stats.disk.used),
        freeFormatted: systemService.formatBytes(stats.disk.free),
        recordingsSizeFormatted: systemService.formatBytes(stats.disk.recordingsSize)
      },
      gpu: gpu.available ? {
        ...gpu,
        memory: gpu.memory ? {
          ...gpu.memory,
          totalFormatted: systemService.formatBytes(gpu.memory.total),
          usedFormatted: systemService.formatBytes(gpu.memory.used),
          freeFormatted: systemService.formatBytes(gpu.memory.free)
        } : undefined
      } : { available: false },
      uptime: stats.uptime,
      uptimeFormatted: systemService.formatUptime(stats.uptime)
    };
 ...
 * CPU usage details
 */
router.get('/cpu', requireOperator, async (req: Request, res: Response) => {
  try {
 ...
 * GPU usage details
 */
router.get('/gpu', requireOperator, async (req: Request, res: Response) => {
  try {
    const gpu = await systemService.getGPUStats();
 ...
 * Disk usage details
 */
router.get('/disk', requireOperator, async (req: Request, res: Response) => {
  try {
    const stats = await systemService.getSystemStats();
 ...
 */
router.get('/processes', requireOperator, async (req: Request, res: Response) => {
  try {
    const pm2Processes = await systemService.getProcesses();
    const ffmpegProcesses = await systemService.getFFmpegProcesses();
...
router.get('/logs', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    // Log activity
    await activityService.logFromRequest(req, 'view_system_logs', {
      resourceType: 'system',
